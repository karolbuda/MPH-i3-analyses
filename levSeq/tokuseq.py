...
def robust_consensus_and_mutations(filtered_reads, reference_seq, output_dir, well_name, threshold=0.5, mixed_threshold=0.25):
    """
    ...
    mixed_threshold: float (minimum fraction for secondary base to detect as mixed)
    Returns: consensus_seq (str), mutations (list of str), has_mixed_bases (bool)
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(len(ref_aligned)):
            ...
            # Get the two most common bases to check for mixed bases
            most_common = Counter(base_counts_no_N).most_common(2)
            most_common_base, most_common_count = most_common[0]
            total_reads = sum(base_counts_no_N.values())
            
            # Check for mixed base (secondary base ≥ mixed_threshold)
            if len(most_common) >= 2:
                second_base, second_count = most_common[1]
                second_fraction = second_count / total_reads
                if second_fraction >= mixed_threshold:
                    has_mixed_bases = True
            
            # Standard consensus calling (majority rule)
        ...
        if not mutations:
            mutations.append("WT")
        return consensus_seq, mutations, has_mixed_bases
...
def main():
    ...
    with open(SUMMARY_CSV, "w", newline="") as csvfile:
        ...
        writer.writerow(["Well", "Consensus", "Mutations", "ReadCount", "MixedBase"])
        for well in sorted(well_reads.keys()):
            reads = well_reads[well]
            if not reads:
                continue
            well_output_dir = os.path.join(OUTPUT_DIR, well)
            consensus_seq, mutations, has_mixed = robust_consensus_and_mutations(reads, ref_seq, well_output_dir, well)
            writer.writerow([well, consensus_seq, ";".join(mutations), len(reads), has_mixed])
